...
\begin{tikzpicture}[
  ...
  \node[system, below=10mm of docs] (cps) {Real Open-Source\\Critical-Infrastructure\\Stack};
  \node[twin, right=14mm of obsA] (twin) {Digital Twin\\(VMs, containers,\\process replicas)};
...
target \cps. Their inputs are heterogeneous: upstream project
documentation and README files in Markdown, deployment and network
diagrams in Visio or Draw.io, configuration files in YAML, TOML, INI,
and HCL, infrastructure-as-code in Terraform, Ansible, and Helm,
Kubernetes manifests, software bills of materials, dependency-lock
files, engineering change logs, and, critically, the operator's
...
(e.g., the deployment diagram references a database replica whose
version or cluster role is not documented), the KB Agents generate a
targeted question for a human operator rather than guessing.
...
commodity services that are tightly coupled to their host OS, a
container for the bulk of microservice and middleware components,
a process-level replica configured with a sanitized copy of the real
deployment's settings, an upstream reference image (e.g., the official
container image for a given open-source project, pinned to the same
version as production), or, for components that cannot be copied
wholesale---closed-source third-party services interacting with the
open-source stack, components whose state volume makes full replication
infeasible, or components whose behavior is observable but whose
implementation is opaque---a behavioral \emph{mock} (as in our Conware
work) whose responses are learned from observed interactions. The agent
then assembles these replicated components into a network topology that
mirrors the real one, populates it with representative data, and exposes
it to the Red Team Agents. The controlled-fidelity aspect of this
process is
...
security-meaningful outcome (e.g., unauthorized read of a patient
records table, privilege escalation to a cluster administrator,
compromise of a secrets store, or arbitrary code execution inside a
message-broker container), and plan a multi-step attack from an initial
attacker position. Atomic steps---individual exploits, misconfigurations,
