...
The PIs plan to introduce modules on the security of open-source
critical-infrastructure stacks explicitly into their graduate and
undergraduate classes, including hands-on lab exercises in which
students find and exploit bugs in the components of a realistic
deployment and propose remediations. The \sysname framework itself,
which is designed to run continuously against reproducible twin
environments, is particularly well-suited to classroom use: students
can be given their own twin instances in which to experiment without
any risk to real production services.
...
three to four ERSP students on developing passive observation modules
and behavioral mock-synthesis techniques for opaque open-source
components, a self-contained and well-scoped portion of Thrust~1 and
Thrust~2. As part of this effort, students will study a representative
set of widely-deployed services, design techniques to recover their
behavioral models from passive observations, and contribute the
resulting modules to the \sysname open-source release. A letter of
collaboration from Ziad Matni, who leads the ERSP program, is
attached to this proposal.

\subsection{Community Outreach}

The PIs will organize a series of security competitions centered on
the security of open-source critical-infrastructure stacks. The PIs
have participated with their hacking team
...
competition (separate from the iCTF) focused specifically on the
security of open-source infrastructure deployments. This competition
will raise awareness, develop participant capabilities, and motivate
students to look more deeply into the complex stacks that underpin
critical services. The competition will be open to the public,
...
will leverage the work of ERSP students, whose focus is on observation
and behavioral modeling of open-source components, to develop
challenge infrastructure for these live competitions.
...
assessment of the open-source software that runs critical
infrastructure.
